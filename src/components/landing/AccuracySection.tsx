...
const AccuracySection = () => {
    return (
        <section id="accuracy" className="py-24 relative bg-white overflow-hidden">
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
                    ...
                    >
                        <div className="space-y-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <span className="text-emerald-600 font-mono text-xs uppercase tracking-widest">System Integrity</span>
                            </div>
                            <h2 className="text-3xl md:text-5xl font-serif font-bold text-zinc-900 tracking-tight leading-tight">
                        ...
                        <div className="pt-8 border-t border-zinc-200">
                             <div className="flex items-center gap-3 text-sm text-zinc-500 font-mono">
                                <div className="flex -space-x-2">
                                     <div className="w-6 h-6 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center text-[10px] text-zinc-500">YT</div>
                                     <div className="w-6 h-6 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center text-[10px] text-zinc-500">S</div>
                                     <div className="w-6 h-6 rounded-full bg-zinc-100 border border-zinc-200 flex items-center justify-center text-[10px] text-zinc-500">+20</div>
                                </div>
                    ...
                    >
                        ...
                        <div className="absolute -inset-1 bg-gradient-to-r from-emerald-500/5 to-blue-500/5 rounded-2xl blur-xl opacity-50 group-hover:opacity-75 transition-opacity duration-500"></div>
                        <ProofWidget />
                    </motion.div>
                </div>
            </div>
...
const FeaturePoint = ({ icon: Icon, title, desc }: { icon: any, title: string, desc: string }) => (
    <div className="flex gap-4 group">
        <div className="w-10 h-10 rounded-lg bg-zinc-50 border border-zinc-200 flex items-center justify-center shrink-0 group-hover:border-emerald-500/30 group-hover:bg-emerald-500/10 transition-all duration-300">
            <Icon className="w-5 h-5 text-emerald-600" />
        </div>
        <div>
            <h3 className="text-zinc-900 font-medium mb-1 text-base group-hover:text-emerald-700 transition-colors">{title}</h3>
...
const ProofWidget = () => {
    ...
    return (
        <div
            className="bg-white border border-zinc-200 rounded-xl overflow-hidden shadow-xl ring-1 ring-black/5"
        ...
        >
            ...
            <div className="p-3 border-b border-zinc-200 bg-zinc-50 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden md:flex items-center gap-2 px-2 py-1 rounded bg-white border border-zinc-200 shadow-sm">
                        <Clock className="w-3 h-3 text-zinc-400" />
                        <span className="text-[10px] font-mono text-zinc-500">T-00:00:00.042</span>
                    </div>

                    <label className="text-[10px] font-medium text-zinc-500 cursor-pointer select-none flex items-center gap-2 hover:text-zinc-900 transition-colors">
                        Show mismatches
                        <button
                            role="switch"
                            ...
                            className={`w-7 h-4 rounded-full relative transition-colors ${filterMismatches ? 'bg-emerald-600' : 'bg-zinc-200 border border-zinc-300'}`}
                        >
                            <motion.div
                                layout
                                className="w-2.5 h-2.5 bg-white rounded-full absolute top-[2px] left-[3px] shadow-sm"
            ...
            {/* Event List */}
            <div className="bg-white min-h-[420px] max-h-[500px] overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-300 scrollbar-track-transparent">
                <LayoutGroup>
                    <motion.div layout className="divide-y divide-zinc-100">
                ...
                {filteredEvents.length === 0 && (
                    ...
                    >
                        ...
                        <p className="text-zinc-400 text-xs font-mono">System reconciled. No anomalies found.</p>
                    </motion.div>
                )}
            </div>

            {/* Footer Status Bar */}
            <div className="bg-zinc-50 border-t border-zinc-200 p-2 flex justify-between items-center text-[10px] text-zinc-500 font-mono">
...
const EventRow = ({
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`group transition-all duration-300 border-l-2 ${isExpanded ? 'bg-zinc-50 border-l-emerald-600' : 'hover:bg-zinc-50 border-l-transparent'}`}
        >
            {/* Row Content */}
            <div
                onClick={onToggle}
            ...
            >
                ...
                <div className="min-w-0">
                    <div className="font-medium text-zinc-900 text-sm truncate flex items-center gap-2">
                        {event.source}
                    ...
                    <div className="text-[10px] text-zinc-500 mt-0.5 hidden md:block font-mono opacity-80">{event.date} • ID: {event.id.split('-')[1]}</div>
                </div>

                {/* Amount (Desktop) */}
                <div className="hidden md:block font-mono text-zinc-700 text-sm text-right md:text-left tracking-tight">
                    {event.amount}
                </div>
                ...
                {/* Expand Button */}
                <button
                    className={`p-1.5 rounded-md hover:bg-zinc-200 text-zinc-400 transition-all duration-300 ${isExpanded ? 'rotate-90 text-zinc-900 bg-zinc-200' : ''}`}
...
const StatusChip = ({ status }: { status: EventStatus }) => {
    const config = {
        matched: { color: 'text-emerald-700 bg-emerald-50 border-emerald-200', label: 'Match' },
        review: { color: 'text-amber-700 bg-amber-50 border-amber-200', label: 'Review' },
        flagged: { color: 'text-rose-700 bg-rose-50 border-rose-200', label: 'Flagged' },
    };
...
const ConfidenceMeter = ({ value }: { value: number }) => {
    ...
    return (
        <div className="flex items-center gap-2 w-24 group/meter cursor-help" title={`Confidence Score: ${value}%`}>
            <div className="flex-1 h-1 bg-zinc-200 rounded-sm overflow-hidden">
                <motion.div
                    initial={{ width: 0 }}
            ...
            <div className="flex border-b border-zinc-200">
                {tabs.map(tab => (
                    <button
                        ...
                        className={`relative flex items-center gap-2 px-4 py-2 text-xs font-medium transition-all duration-300 outline-none focus:bg-zinc-100 ${
                            activeTab === tab.id
                                ? 'text-zinc-900'
                    ...
                    >
                        {activeTab === tab.id && (
                            <motion.div
                                ...
                                className="absolute bottom-0 left-0 right-0 h-[1px] bg-emerald-600 shadow-[0_0_10px_rgba(16,185,129,0.3)]"
                            />
                        )}
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'trace' && (
                        ...
                        >
                            <div className="relative pl-2">
                                {/* Vertical Line */}
                                <div className="absolute left-[5.5px] top-2 bottom-2 w-[1px] bg-zinc-200"></div>
                                ...
                                {event.trace.map((node, i) => (
                                    ...
                                    >
                                        <div className={`z-10 w-2 h-2 rounded-full border border-white transition-all duration-300 ${i === event.trace.length - 1 ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)] scale-110' : 'bg-zinc-300'}`}></div>
                                        <div className="flex-1 flex justify-between items-center p-2 rounded border border-transparent hover:border-zinc-200 hover:bg-zinc-50 transition-all cursor-crosshair">
                                            <span className="text-xs text-zinc-700 font-medium">{node.step}</span>
                                            <div className="flex items-center gap-3">
                                                <span className="text-[10px] font-mono text-zinc-500 group-hover/node:text-emerald-600/70 transition-colors">#{node.hash}</span>
                                                <span className="text-[10px] font-mono text-zinc-400">{node.time}</span>
                                            </div>
                                        </div>
                                    </motion.div>
                                ))}
                    ...
                    {activeTab === 'explain' && (
                        ...
                        >
                            <div className="p-3 rounded bg-zinc-50 border border-zinc-200">
                                <h4 className="text-[10px] font-mono text-emerald-600 uppercase tracking-wider mb-2 flex items-center gap-2">
                                    <FileText className="w-3 h-3" /> System Reasoning
                                </h4>
                                <p className="text-xs text-zinc-700 leading-relaxed font-mono opacity-90">{event.explanation}</p>
                            </div>
                            {event.raw_data && (
                                <div className="p-3 rounded bg-zinc-50 border border-zinc-200">
                                    <h4 className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider mb-1">Raw Payload</h4>
                    ...
                    {activeTab === 'resolve' && (
                        ...
                        >
                            {resolvedState === 'none' ? (
                                <div className="space-y-4">
                                    <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-100 rounded">
                                    ...
                                    <div className="flex gap-3">
                                        <Button
                                            ...
                                            className="bg-emerald-600 hover:bg-emerald-500 text-white flex-1 h-9 text-xs font-semibold tracking-wide shadow-sm"
                                        >
                                        ...
                                        <Button
                                            ...
                                            variant="outline"
                                            className="border-rose-200 text-rose-600 bg-rose-50 hover:bg-rose-100 hover:text-rose-700 flex-1 h-9 text-xs"
                                        >
                                ...
                                <motion.div
                                    ...
                                    className="flex flex-col items-center justify-center py-8 text-emerald-600 bg-emerald-50 rounded border border-emerald-100"
                                >
                                    <div className="w-12 h-12 rounded-full bg-emerald-100 flex items-center justify-center mb-3 ring-1 ring-emerald-200 shadow-sm">
                                        <Check className="w-6 h-6" />
                                    </div>
                                    <p className="font-mono text-sm font-bold tracking-tight">TRANSACTION LOCKED</p>
                                    <p className="text-[10px] text-emerald-600/60 font-mono mt-1">Hash: 0x99a...f7b2 • Timestamp: {new Date().toLocaleTimeString()}</p>
                                    <button
                                        onClick={(e) => { e.stopPropagation(); setResolvedState('none'); }}
                                        className="text-[10px] text-zinc-500 mt-4 hover:text-zinc-900 underline decoration-zinc-300 underline-offset-4"
                                    >
                                        Undo Action
                                    </button>
                                </motion.div>
                                ...
                                >
                                    <div className="space-y-1">
                                        ...
                                        <select className="w-full bg-white border border-zinc-200 rounded p-2 text-xs text-zinc-900 focus:ring-1 focus:ring-rose-500/50 outline-none">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <textarea
                                            className="w-full bg-white border border-zinc-200 rounded p-2 text-xs text-zinc-900 h-16 focus:ring-1 focus:ring-rose-500/50 outline-none resize-none placeholder:text-zinc-400"
                                            placeholder="Add context for audit trail..."
                                        ></textarea>
                                    </div>
                                    <div className="flex justify-end gap-2">
                                        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setResolvedState('none'); }} className="text-zinc-500 h-8 text-xs hover:text-zinc-900 hover:bg-zinc-100">Cancel</Button>
                                        <Button size="sm" className="bg-rose-600 hover:bg-rose-700 h-8 text-xs shadow-sm">Submit Flag</Button>
                                    </div>
                                </motion.div>
                            )}
                        </motion.div>
