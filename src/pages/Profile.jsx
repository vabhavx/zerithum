...
export default function Profile() {
  ...
  const { data: connectedPlatforms = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return base44.entities.ConnectedPlatform.filter({ user_id: currentUser.id }, "-connected_at", 100);
    },
    staleTime: 1000 * 60 * 2,
  });
      ...
      <header className="mb-6 flex flex-col gap-4 border-b border-white/10 pb-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/70">
            Manage account identity, secure access, and connected financial data sources.
          </p>
        </div>
